...
public:
  ...
  explicit Description(const unsigned long size = 0, const std::string & expr = "") : std::vector<std::string>(size)
  { 
...
{
public:
  explicit NumericalPoint(const unsigned long size = 0, const double expr = 0.) : std::vector<double>(size)
  { 
    for (unsigned long i = 0; i < size; ++i)
    {
      (*this)[i] = expr;
...
std::string randExp(int n, int dim)
{
    if (n==0)
        return randVar(dim);
    else {
...
//       formulas = Description(1,"cos(x1)*sin(x1)");
      formulas = Description(1,"((x2)*(x1))^(((x2)*(x1))/((-4*x2)))");
